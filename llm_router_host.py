...
def _auth_token(request: dict, env_get: Callable[[str], str | None],
                default_env: str) -> tuple[str | None, dict | None]:
    auth_env = request.get("auth_env") or default_env
    token = env_get(auth_env)
    if not token:
        return None, _err("auth_error", 0, 0, f"env var {auth_env!r} unset")
    return token, None
...
def _openai_messages_to_anthropic(messages: list[dict]) -> tuple[list[dict], str | None]:
    out, system_parts = [], []
...
def make_anthropic_async_call_provider(
    ...
    async def call(request: dict) -> dict:
        token, err = _auth_token(request, _env_get, "ANTHROPIC_API_KEY")
        if err is not None:
            return err
        messages, system = _openai_messages_to_anthropic(request.get("messages") or [])
        body: dict[str, Any] = {
        ...
        url = (request.get("base_url") or "https://api.anthropic.com/v1").rstrip("/") + "/messages"
        timeout = (request.get("timeout_ms") or int(timeout_s * 1000)) / 1000.0
        t0 = time.monotonic()
        try:
...
def _parse_gemini_response(data: dict, status: int, latency: int) -> dict:
    ...
    if not candidates:
        return _err("content_filter" if data.get("promptFeedback") else "bad_response",
                    status, latency, "no candidates in response")
    cand = candidates[0]
...
def make_google_async_call_provider(
    ...
    async def call(request: dict) -> dict:
        token, err = _auth_token(request, _env_get, "GEMINI_API_KEY")
        if err is not None:
            return err
        model = (request.get("offer") or {}).get("wire_model_id") or request["served_model_id"]
        ...
        if tools:
            body["tools"] = tools
        base = (request.get("base_url") or "https://generativelanguage.googleapis.com/v1beta").rstrip("/")
        url = f"{base}/{quote(model_path, safe='/')}:generateContent?key={quote(token, safe='')}"
